...
- [ ] 1.1 创建 Maven 多模块项目结构（父 POM + 6 个子模块）
- [ ] 1.2 配置 Spring Boot 3 依赖和插件
- [ ] 1.3 配置 GraalVM JS、Jackson、SnakeYAML、OkHttp 依赖
- [ ] 1.4 创建 data/ 目录结构和 .gitkeep 文件

## 2. module-common 共享层

- [ ] 2.1 创建 ProxyNode 模型类
- [ ] 2.2 创建 ClashConfig 模型类
- [ ] 2.3 创建统一异常定义（BusinessException 等）
- [ ] 2.4 创建 YAML 解析工具类
- [ ] 2.5 创建 Base64 解码工具类
- [ ] 2.6 创建 ID 生成工具类
...
- [ ] 3.1 创建 Subscription 模型类
- [ ] 3.2 定义 SubscriptionRepository 接口
- [ ] 3.3 实现 JsonFileSubscriptionRepository
- [ ] 3.4 定义 SubscriptionService 接口
- [ ] 3.5 实现 SubscriptionService（CRUD 逻辑）
- [ ] 3.6 实现订阅获取逻辑（HTTP 请求 + User-Agent）
- [ ] 3.7 实现订阅解析逻辑（YAML / Base64 自动检测）
- [ ] 3.8 实现订阅缓存逻辑

## 4. module-mihomo 实例管理

- [ ] 4.1 创建 MihomoInstance 模型类
- [ ] 4.2 定义 MihomoInstanceRepository 接口
- [ ] 4.3 实现 JsonFileMihomoInstanceRepository
- [ ] 4.4 创建 MihomoHttpClient（HTTP API 客户端）
- [ ] 4.5 定义 MihomoService 接口
- [ ] 4.6 实现 MihomoService（CRUD + 健康检查 + 配置推送）

## 5. module-processor 配置处理

- [ ] 5.1 定义 ConfigProcessor 接口
- [ ] 5.2 创建 ProcessingContext 模型
...
- [ ] 5.8 定义 Pipeline 配置模型
- [ ] 5.9 实现 Pipeline 执行引擎

## 6. module-scheduler 定时任务

- [ ] 6.1 创建 ScheduledTask 模型类
