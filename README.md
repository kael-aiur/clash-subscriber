...
## 为什么需要 Clash Subscriber

当你在服务器或路由器上部署 Clash/Mihomo 给全家设备共享使用时，往往会遇到这些问题：

**订阅切换麻烦** — 为了稳定上网，通常会购买多个服务商的订阅作为备份。但每次某个订阅出问题，都要手动登录服务器切换到另一个订阅，操作繁琐且不够及时。

**配置难以定制** — 订阅服务商提供的配置是固定的，想调整分流规则或扩展节点组很困难。手动修改的话，每次订阅更新都会被覆盖；用脚本自动改又要自己写脚本、管理脚本，门槛很高。

**更新维护费力** — 订阅节点需要定期更新，不然新节点用不上。手动定时更新很麻烦，有时候订阅链接都变了，还要重新登录服务器修改配置。

**转发规则看不懂** — 订阅服务商提供的规则配置往往很复杂，不清楚自己的请求到底走了哪个节点、经过了怎样的转发路径，排查问题时无从下手。

Clash Subscriber 针对以上痛点提供了解决方案：

- **多订阅源合并，一个挂了自动有其他顶上** — 支持添加多个订阅源作为节点来源，通过可视化配置将所有订阅的节点自动合并到同一个配置中。只要有一个订阅厂商可用，整体服务就不会中断，不再需要手动切换。

- **可视化配置组合策略 + 脚本灵活定制** — 提供可视化的配置组合编辑器，可以灵活定义代理组、规则组和节点筛选策略；同时也支持 JavaScript 脚本编写更复杂的合并和过滤逻辑。配置组合和脚本统一在 Web 管理台中维护，告别散落的脚本文件。

- **构建流水线自动定时刷新** — 支持创建构建流水线任务，按流水线步骤组合订阅源、脚本处理和目标 Mihomo 实例，并可绑定 Cron 表达式自动定时触发。订阅更新和配置推送全自动完成，无需手动干预。

- **转发路径可视化查询** — 输入任意域名即可查看该域名的完整转发流程：命中哪条规则、匹配到哪个策略组、最终从哪个出口节点出去。不再对着复杂的规则配置发呆。
